...
.issue-card { border: 1px solid #e5e7eb; border-radius: 12px; padding: 20px; margin-bottom: 12px; background: #fff; }
    ...
    if (isMultiPage) {
      ...
      for (const [pageName, pageIssues] of Object.entries(byPage)) {
        ...
        for (const issue of pageIssues) {
          globalIndex += 1;
          typeSectionsHtml.push(await renderIssueCard(issue, globalIndex, allIssues));
        }
...
<body>
  <header class="report-header">
    ...
    <div class="meta-grid">
      ...
      <div><strong>\u7075\u654F\u5EA6\uFF1A</strong>${meta.threshold || 0.23}</div>
  ...
  <footer class="footer">
    <p>\u7531 AI UI \u8D70\u67E5\u52A9\u624B v2.2.0 \u81EA\u52A8\u751F\u6210 | ${escapeHtml(timestamp)}</p>
  </footer>
</body>
</html>`;
...
async function renderIssueCard(issue, index, allIssues) {
