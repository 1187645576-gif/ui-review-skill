...
async function runReview(options) {
  ...
  const htmlContent = await generateHtmlReport(analysisResult, reportMeta);
  const wordContent = await generateWordReport(analysisResult, reportMeta);
...
async function runCli(argv) {
  ...
  if (!args.design || !args.dev) {
    console.log(`
用法：node src/index.js --design <设计稿图片> --dev <开发稿图片> [选项]
...
module.exports = { runReview, runCli };
